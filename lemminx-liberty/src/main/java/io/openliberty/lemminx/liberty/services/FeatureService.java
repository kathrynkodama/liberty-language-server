...
public class FeatureService {
  ...
  public List<Feature> getFeatures(String libertyVersion, int requestDelay, String documentURI) {
    // LOGGER.info("Getting features for version: " + libertyVersion);
    ...
    LOGGER.info("-----fetching installed features: " + installedFeatures.size());
  ...
  private List<Feature> getInstalledFeaturesList(String documentURI) {
    ...
    try {
      ...
      File tempFeaturesList = File.createTempFile("featureslist", ".xml", featureListJAR.toFile().getParentFile());
      String[] cmd = { "java", "-jar", featureListJAR.toAbsolutePath().toString(), tempFeaturesList.getAbsolutePath() };
